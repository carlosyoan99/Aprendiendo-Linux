...
- Sesión 2026-09-03 (v66): **Auditoría completa del vault** — 1) Frontmatter: 606 OK / 612, solo exentas con avisos (README, CLAUDE, AGENTS sin frontmatter; Log.md, Log-2026.md, SKILL.md sin hashtag — todas excluidas por hooks). 2) Wikilinks: 1 roto [[dmesg]] en journald → corregido a [[Coreutils y util-linux]] (0 rotos). 3) Huérfanas: solo SKILL.md externo de .agents (no es del vault). 4) Duplicado detectado: journald existía en 03 (sistema, 93L) y 08 (programa, 357L) → fusionado: contenido único de 03 (filtros _TRANSPORT/_COMM/_SYSTEMD_UNIT, Storage/Seal/ForwardToSyslog en journald.conf, rotate/export) integrado en 08; 03 eliminada. 5) Cobertura por carpeta: 12/12 carpetas, 0 borradores de contenido (solo plantillas), 12 categorías válidas. Stats → 611 notas.
- Sesión 2026-09-03 (v65): **Seguridad para servidores** — 1 nota nueva: [[Hardening de Nginx]] (08, 320 líneas: checklist, ocultar versión, TLS 1.2/1.3 + HSTS preload, 6 cabeceras de seguridad + CSP, rate limiting login/wp-login, deny rutas sensibles (.git/.env) + auth_basic, métodos HTTP, PHP-FPM endurecido, fail2ban jails nginx, bloqueo de bots, límites anti-DoS/slowloris, auditoría con testssl.sh/sslyze, troubleshooting 6 casos). [[SSH Hardening]] expandida (366→483: sección 11 2FA PAM/TOTP con google-authenticator y AuthenticationMethods publickey,keyboard-interactive; 12 límites de sesión MaxStartups/LoginGraceTime/ClientAliveInterval/AllowUsers; 13 chroot SFTP con Match Group y regla root:root; 14 sshguard con comparativa vs fail2ban; tabla comparativa actualizada). [[journald]] expandida (264→357: monitoreo remoto systemd-journal-remote/upload/gatewayd, arquitectura cliente-servidor, TLS con CA propia, notas de producción, tabla comparativa ampliada con Loki/Graylog). MoC actualizado (descripciones SSH Hardening/journald). Stats → 612 notas.
- Sesión 2026-09-03 (v64): **Nota nueva OBS Studio** — [[OBS Studio]] (08, ~230 líneas): instalación multi-distro + Flatpak, v4l2loopback/obs-vkcapture, workflow completo de 7 pasos (autoconfiguración → escenas → fuentes → audio → salida → emitir), buenas prácticas de escenas/fuentes (nested scenes, prefijos), cadena de audio completa (RNNoise, gate, compresor, multi-track 1-4), atajos de teclado, replay buffer, cámara virtual, plugins esenciales, ajustes de grabación (MKV→remux) y streaming por plataforma (Twitch/YouTube/Kick), troubleshooting 8 casos. Cross-link con [[Grabación de pantalla y streaming]] (encoders/alternativas). MoC actualizado. Stats → 611 notas.
- Sesión 2026-09-03 (v63): **4 notas cortas 08 expandidas** — [[FRRouting]] (88→155: activar daemons en /etc/frr/daemons, ejemplos BGP/OSPF completos en vtysh, comandos de verificación show ip route/bgp summary, frr-reload.py en caliente, troubleshooting 6 casos). [[OnlyOffice]] (89→137: repo oficial Debian/Fedora/Arch, CLI desktopeditors, integración Nextcloud/WebDAV, plugins, atajos completos, troubleshooting 6 casos). [[Vivaldi]] (89→138: tab stacking/tiling/workspaces/paneles, gestos ratón, Vivaldi Sync E2E, privacidad, flags CLI Wayland/GPU, comparativa con Opera, troubleshooting 6 casos). [[WPS Office]] (88→124: CLI wps/et/wpp/wpspdf, paquetes mui/diccionarios, fuentes mscorefonts, mime associations, troubleshooting 6 casos). Stats sin cambio (610 notas).
- Sesión 2026-09-03 (v62): **Notas cortas 08 expandidas** — [[suckless]] (58→150: flujo compilar/config.h/make clean install, dependencias, aplicar parches con patch -p1 y .rej, sesión DWM+slstatus+slock, troubleshooting). [[Foot]] (88→185: foot.ini completo por secciones, atajos por defecto, modo servidor footclient, comparativa con Alacritty/Kitty/WezTerm/GNOME Terminal, troubleshooting expandido). Stats sin cambio (610 notas).
...
- Sesión 2026-09-03 (v56): **1 nota nueva** — [[Grabación de pantalla y streaming]] (08, OBS Studio, captura X11/Wayland, codificadores VAAPI/NVENC/AV1, wf-recorder, gpu-screen-recorder, ffmpeg CLI, baja latencia streaming, Twitch/YouTube). MoC actualizado. Stats → 609 notas.
